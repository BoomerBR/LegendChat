...
public class Listeners_old implements Listener {
	...
	private static final HashMap<PlayerChatEvent,Boolean> chats = new HashMap<>();
	
	public static HashMap<PlayerChatEvent, Boolean> getChats() {
		HashMap<PlayerChatEvent,Boolean> clone = new HashMap<>();
		clone.putAll(chats);
		return clone;
	}
	...
	private boolean hasAnyPermission(Player sender) {
		...
		if(sender.hasPermission("legendchat.admin.unmuteall"))
			return true;
		return sender.hasPermission("legendchat.admin.reload");
	}
}
